...
# %% SHIT
## nope, this stepwise fitting does NOT generalize!
#
#traincols = np.zeros(len(pokenames))
#is_converged = False
#bestfit      = 0.5 # prior. analysis is set up so that this is indeed the baseilne prediction
#    
#while not is_converged:
#    
#    fit_of_shifts = np.zeros(len(pokenames))
#    
#    for ind in range(len(pokenames)):
#        tcols = traincols.copy()
#        
#        if tcols[ind] == 0:
#            tcols[ind] = 1
#        else:
#            tcols[ind] = 0
#            
#        tcols_logical = [bool(tcols[ind_]) for ind_ in range(len(tcols))]
#        tmat = usdtrain.copy()
#        tmat = tmat[:,tcols_logical]
#        vmat = usdvalid.copy()
#        vmat = vmat[:,tcols_logical]
#        
#        if any(tcols_logical):
#            trainmdl = lm.LogisticRegression(penalty='none',fit_intercept=False,solver='lbfgs',max_iter=1000) 
#            # use a 50-50 prior. We're assuming that any sampling biases will be minimal (and therefore not need correction), and the lack of an offset term makes the regression coefficients easier to interpret
#            # dataset is small enough that lbfgs won't cause any massive problems
#        
#            trainmdl.fit(tmat,WLtrain)
#            fit_of_shifts[ind] = trainmdl.score(vmat,WLvalid)
#        else:
#            fit_of_shifts[ind] = 0.5
#        
#    maxfit = np.max(fit_of_shifts)
#    maxind = np.argmax(fit_of_shifts)
#    
#    present_ind = traincols[maxind]
#        
#    if maxfit > bestfit:
#        bestfit = maxfit
#        if present_ind == 0:
...
#tmat = usdtrain.copy()
#tmat = tmat[:,tcols_logical]
#testmat = usdtest.copy()
#testmat = testmat[:,tcols_logical]
#testpokes = [pokenames[ind] for ind in range(len(tcols_logical)) if tcols_logical[ind]]
#
#testmdl = lm.LogisticRegression(penalty='none',fit_intercept=False,solver='lbfgs',max_iter=1000)
#testmdl.fit(tmat,WLtrain)
#
#fitval = testmdl.score(testmat,WLtest)
    
# %% report overall goodness-of fit
# use pseudo-R2
# use the "called it" rate as a more intuitive measure
# 